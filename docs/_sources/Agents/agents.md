...
在哲学，尤其是**伦理学、心灵哲学和行动哲学**中：Agent指的是一个有能力做出自主决定并采取行动的存在。
...
| 级别 | 描述                           | 名称       | 示例模式                                           |
| ---- | ------------------------------ | ---------- | -------------------------------------------------- |
| ☆☆☆  | 智能体输出不会影响程序流程     | 简单处理器 | `process_llm_output(llm_response)`                 |
| ★☆☆  | 智能体输出决定基本控制流       | 路由器     | `if llm_decision(): path_a() else: path_b()`       |
| ★★☆  | 智能体输出决定函数执行         | 工具调用器 | `run_function(llm_chosen_tool, llm_chosen_args)`   |
| ★★★  | 智能体输出控制迭代和程序继续   | 多步骤代理 | `while llm_should_continue(): execute_next_step()` |
| ★★★  | 一个智能体可以启动另一个智能体 | 多智能体   | `if llm_trigger(): execute_agent()`                |
...
**工具集成：**
 调用工具（如天气 API）的能力使天气智能体够超越静态知识，检索实时数据，这是许多 AI 智能体的关键特性。
...
| 记忆整合   | 用户之前提到他更喜欢 Python，所以我将提供 Python 示例        |
...
- [扣子 Agent](https://www.coze.cn/space-preview)
